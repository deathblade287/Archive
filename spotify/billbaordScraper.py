...
def get_billboard(date: str = '2019-11-11', count: int = 5) -> pd.DataFrame:
    ...
    df_billboard = pd.DataFrame(
        columns=["title", "artist", "rank", "weeks_on_charts"])
    ...
    for result in results:
        title = result.find(
            'span', class_="chart-element__information__song text--truncate color--primary").text
        artist = result.find(
            'span', class_="chart-element__information__artist text--truncate color--secondary").text
        rank = result.find('span', class_="chart-element__rank__number").text
        weeks = result.find(
            'span', class_="chart-element__meta text--center color--secondary text--week").text

        df_billboard = df_billboard.append(
            {'title': title, 'artist': artist, 'rank': rank, 'weeks_on_charts': weeks}, ignore_index=True)

    return df_billboard[:count]


# String manipulation / cleaning method
def _remove_accents(input_str): return ''.join(
    (c for c in unicodedata.normalize('NFKD', input_str) if not unicodedata.combining(c)))


def _clean_string(s): return set(
    re.sub(r'[^\w\s]', '', _remove_accents(s)).lower().split())
# Measures difference in letters - to analyze accuracy of search results


def _jaccard(set1, set2): return float(
    len(set1 & set2)) / float(len(set1 | set2))
